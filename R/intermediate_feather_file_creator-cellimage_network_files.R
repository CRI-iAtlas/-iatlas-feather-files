build_cellimage_network_files <- function(){

  hgnc_labels <- "syn21781366" %>%
    synapse_feather_id_to_tbl %>%
    dplyr::select("value" = "Variable", "x", "y")

  hgnc_scaffold <- "syn21782167" %>%
    synapse_feather_id_to_tbl %>%
    dplyr::select("from" = "From", "to" = "To", "label" = "interaction") %>%
    dplyr::mutate("number" = 1:dplyr::n()) %>%
    tidyr::pivot_longer(-c("label", "number"))
  ...
  entrez_labels <- hgnc_labels %>%
    ...
    dplyr::select("node" = "new_value", "x", "y")

  synapse_store_feather_file(
    entrez_scaffold,
    "cellimage_scaffold.feather",
    "syn23518511"
  )

  synapse_store_feather_file(
    entrez_labels,
    "cellimage_node_labels.feather",
    "syn23518511"
  )
